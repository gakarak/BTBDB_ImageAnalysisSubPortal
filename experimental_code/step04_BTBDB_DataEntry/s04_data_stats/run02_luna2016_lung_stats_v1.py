...
if __name__ == '__main__':
    fidx = '/media/data10T_1/datasets/luna16/data_nii/idx.txt'
    wdir = os.path.dirname(fidx)

    dataCSV = pd.read_csv(fidx)
    pathImgs = np.array([os.path.join(wdir, xx) for xx in dataCSV['path_img']])
    pathMsks = np.array([os.path.join(wdir, xx) for xx in dataCSV['path_msk']])
    numSmpls = len(pathMsks)

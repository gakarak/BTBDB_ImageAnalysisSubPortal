...
def _ext_resize_lung_mask(path_nii_inp, inp_shape, out_shape):
    inp_shape_str = 'x'.join([str(x) for x in inp_shape])
    out_shape_str = 'x'.join([str(x) for x in out_shape])
    path_nii_out  = path_nii_inp.replace('-{}.nii'.format(inp_shape_str), '-{}.nii'.format(out_shape_str))
    if os.path.isfile(path_nii_out):
        logging.warning('\t\t\t\t[!!!] resized mask exist, skip... [{}]'.format(path_nii_out))
    else:
        msk_nii_out = resize_nii(path_nii_inp, out_shape, parOrder=0)
        nib.save(msk_nii_out, path_nii_out)
        # print('-')
    # print('-')
...
def task_process_lung_mask_and_resize(pdata):
    ...
    if os.path.isfile(path_out):
        ...
        nii_out = _clone_nifti(msk3d_dilated, path_lung_mask)
        nib.save(nii_out, path_out)
        inp_shape = msk3d_dilated.shape
    nii_img = nib.load(path_lung_mask)
    inp_shape = nii_img.shape
    for ss in lst_ext_sizes:
        out_shape = list(inp_shape)
        out_shape[-1] = ss
        out_shape = tuple(out_shape)
        logging.info('\t\t\t[{}/{}] resize ext masks from {} -> {}'.format(idx, num_idx, inp_shape, out_shape))
        _ext_resize_lung_mask(path_out, inp_shape, out_shape)
        print('-')
    dt = time.time() - t1
    logging.info('\t\t[{}/{}] done... dt ~ {:0.2f} (s)'.format(idx, num_idx, dt))
    return True
...
if __name__ == '__main__':
    ...
    if (num_lungs < 2) or (num_threads < 2):
        lst_res = [task_process_lung_mask_and_resize(xx) for xx in lst_task_data]
    else:
        pool = mp.ProcessPoolExecutor(max_workers=num_threads)
        lst_res = list(pool.map(task_process_lung_mask_and_resize, lst_task_data))

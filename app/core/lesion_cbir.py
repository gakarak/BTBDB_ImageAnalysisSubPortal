...
def _task_evaluate_ldsc(pdata):
    ...
    if ldsc.desc_in_json_file(path_jsn):
        ...
        cur_desc = ldsc.calc_desc(path_sgm, path_les)
        json_data = None
...
def _task_simple_cbir(pdata):
    ...
    case_json = open(lst_fjson[dsc_idx], 'w')
    case_json.write(json.dumps(case_json_data, indent=4, sort_keys=True))
    case_json.close()
    return True


def run_precalculate_lesion_dsc_for_db(path_db:str, num_threads:int):
    data_dir = path_db
    ...
    #
    logging.info(' [*] pre-calculating lesion-cbir descriptors: #tasks/#threads = {}/{}'
                 .format(len(lst_task_data_ldsc), num_threads))
    t1 = time.time()
    ...
    logging.info(' [*] find similar cases for every series: #tasks/#threads = {}/{}'.format(len(lst_task_data_cbir),
                                                                                            num_threads))
    t1 = time.time()
    lst_ret_cbir = []
